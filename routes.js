...
const addExercise = (userData, done)=>{
  let exercise = new Exercise({
    ...
    date: validDate(userData.date)? 
      new Date(userData.date) : 
      new Date()
  })
  exercise.save((err, exerciseData)=>{
    if(err) return done(err);
    User.findById(exerciseData.userId, (err,data)=>{
...
const getExercises = (userData, done)=>{
  let limit = validInt(userData.limit)? 
    parseInt(userData.limit): 0;
  let fromDate = validDate(userData.from) ? 
    new Date(userData.from): minDate;
  let toDate = validDate(userData.to) ? 
    new Date(userData.to): maxDate;
